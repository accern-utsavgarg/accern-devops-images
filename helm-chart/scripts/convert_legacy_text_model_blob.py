...
# Update the sentiment model blobs to work with model versioning updates.
# Assumes that xyme config is already set (setup_config.py)
# Saves the existing model as "init" model version
...
def update_blob_dir(zipdirname: str) -> Optional[int]:
    ...
    for fname in os.listdir(zipdirname):
        to_path = None
        if is_int(fname):
            shutil.rmtree(os.path.join(zipdirname, fname))
        elif fname == MODEL_DIR_NAME:
        ...
        if to_path:
            ...
            for folder in os.listdir(os.path.join(zipdirname, fname)):
                shutil.move(os.path.join(from_path, folder), to_path)
            os.rmdir(from_path)
...
if len(ALL_VERSIONS) > 0:
    ...
    LEGACY_MODEL_BLOB.convert_model()
